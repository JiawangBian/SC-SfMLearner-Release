...
camera_view_choices = YaakIterableDataset.camera_views() + ['all']

parser = argparse.ArgumentParser(
    description='Train the structure from motion learner on the Yaak dataset, '
                'using an efficient data loader (YaakIterableDataset {}).'.format(
        YaakIterableDataset.version()
    ),
...
parser.add_argument(
    ...
    default=25,
    type=int,
    help='Maximum number of iterations in the training set (per epoch).'
)
parser.add_argument(
    '--max-val-iterations',
    default=5,
    type=int,
    help='Maximum number of iterations in the validation set.'
)
...
parser.add_argument(
    ...
    type=float, choices=[0.25, 0.5, 0.75, 1.0],
    default=0.5,
    metavar='M',
    help='Scaling factor applied to the frames along x and y axes.'
)
...
# ----------------------------------------------------------------------------------------------------------------------
# In the TOML file [ dataset ]
# ----------------------------------------------------------------------------------------------------------------------

# train.dataset.rootpath
# val.dataset.rootpath
...
# train.dataset.camera_calibration
# val.dataset.camera_calibration
...
# train.dataset.drive_ids
# val.dataset.drive_ids
...
# train.dataset.camera_view
# val.dataset.camera_view
...
# ----------------------------------------------------------------------------------------------------------------------
# In the TOML file [ configuration.frame ]
# ----------------------------------------------------------------------------------------------------------------------

# train.configuration.frame.frame_count
# val.configuration.frame.frame_count
parser.add_argument('--video-clip-length', type=int, metavar='N', help='Video clip length used for training the model.', default=3)

# train.configuration.frame.frame_step
# val.configuration.frame.frame_step
parser.add_argument('--video-clip-step', type=int, metavar='N', help='Video clip step used for training the model.', default=1)

# ----------------------------------------------------------------------------------------------------------------------
# In the TOML file [ configuration.sampling.speed, dataset.camera_view, configuration.return.telemetry ]
# ----------------------------------------------------------------------------------------------------------------------

# train.configuration.sampling.speed
# train.configuration.return.telemetry
# The camera_view is defined as "cam_rear" in the dataset class.
parser.add_argument(
    '--telemetry-data-params-train',
    default='{"minimum_speed": 8.0, "camera_view": "cam_rear", "return_telemetry": "True"}',
    type=str, help='Parameters used to load telemetry data from the JSON files (e.g., metadata.json). This data is used in the training stage.')

# val.configuration.sampling.speed
# val.configuration.return.telemetry
# The camera_view is defined as "cam_rear" in the dataset class.
parser.add_argument(
    '--telemetry-data-params-val',
    default='{"minimum_speed": 8.0, "camera_view": "cam_rear", "return_telemetry": "True"}',
    type=str,
    help='Parameters used to load telemetry data from the JSON files (e.g., metadata.json). This data is used in the validation stage.'
)
